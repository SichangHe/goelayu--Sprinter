...
class Trace:
                    ...
                    if "method" in event and "params" in event:
                        if self.start_time is None:
                            if (
                                ...
                                and "timestamp" in event["params"]
                            ):
                                self.start_time = (
                                    event["params"]["timestamp"] * 1000000.0
                                )
    ...
    def FilterTraceEvent(self, trace_event):
        ...
        if (
            ...
            or cat == "__metadata"
            or cat.find("devtools.timeline") >= 0
            or cat.find("blink.feature_usage") >= 0
            or cat.find("blink.user_timing") >= 0
            or cat.find("loading") >= 0
            or cat.find("navigation") >= 0
            or cat.find("rail") >= 0
            or cat.find("netlog") >= 0
            or cat.find("v8") >= 0
        ):
    ...
    def ProcessTraceEvent(self, trace_event):
        ...
        if (
            ...
            if (
                "args" in trace_event
                and "data" in trace_event["args"]
                and "inMainFrame" in trace_event["args"]["data"]
                and trace_event["args"]["data"]["inMainFrame"]
            ):
                keep = True
            ...
            if (
                "name" in trace_event
                and trace_event["name"].find("navigationStart") >= 0
            ):
            ...
            if (
                ...
                and trace_event["name"] in ["navigationStart", "fetchStart"]
            ):
        ...
        if (
            cat == "__metadata"
            and "name" in trace_event
            and trace_event["name"] == "process_labels"
            and "pid" in trace_event
            and "args" in trace_event
            and "labels" in trace_event["args"]
            and trace_event["args"]["labels"].startswith("Subframe:")
        ):
            self.cpu["subframes"].append(str(trace_event["pid"]))
        if (
            cat == "__metadata"
            and "name" in trace_event
            and trace_event["name"] == "thread_name"
            and "args" in trace_event
            and "name" in trace_event["args"]
            and trace_event["args"]["name"] == "CrRendererMain"
        ):
            thread = "{0}:{1}".format(trace_event["pid"], trace_event["tid"])
            if thread not in self.cpu["main_threads"]:
                self.cpu["main_threads"].append(thread)
                        ...
                        if event["name"].startswith("NavStartToLargestContentfulPaint"):
                            ...
                            try:
                                if "args" in event:
                                    ...
                                    if (
                                        "data" in event["args"]
                                        and "candidateIndex" in event["args"]["data"]
                                    ):
                                        if isinstance(
                                            event["args"]["data"]["candidateIndex"], int
                                        ):
                                            key += ".{0:d}".format(
                                                event["args"]["data"]["candidateIndex"]
                                            )
                                        elif isinstance(
                                            event["args"]["data"]["candidateIndex"], str
                                        ):
                                            key += (
                                                "."
                                                + event["args"]["data"][
                                ...
                                logging.exception(
                                    "Error processing user timing event key"
                                )
    ...
    def ProcessTimelineTraceEvent(self, trace_event):
        ...
        if (
            ...
            and "name" in trace_event["args"]
            and trace_event["args"]["name"] == "CrRendererMain"
            and thread not in self.cpu["main_threads"]
        ):
            self.cpu["main_threads"].append(thread)
        ...
        if (
            "args" in trace_event
            and "data" in trace_event["args"]
            and thread not in self.ignore_threads
        ):
            if "url" in trace_event["args"]["data"] and trace_event["args"]["data"][
                "url"
            ].startswith("http://127.0.0.1:8888"):
                self.ignore_threads[thread] = True
            if (
                self.cpu["main_thread"] is None
                or "isMainFrame" in trace_event["args"]["data"]
            ):
                if (
                    "isMainFrame" in trace_event["args"]["data"]
                    and trace_event["args"]["data"]["isMainFrame"]
                ) or (
                    trace_event["name"] == "ResourceSendRequest"
                    and "url" in trace_event["args"]["data"]
                ):
        ...
        if (
            self.cpu["main_thread"] is not None
            and thread not in self.threads
            and thread not in self.ignore_threads
            and trace_event["name"] != "Program"
        ):
            self.threads[thread] = {}
        ...
        if thread in self.threads and (
            ...
            if trace_event["name"] not in self.threads[thread]:
                self.threads[thread][trace_event["name"]] = self.event_names[
                    trace_event["name"]
                ]
            e = None
            if trace_event["ph"] == "E":
                ...
                if (
                    trace_event["name"]
                    in ["EvaluateScript", "v8.compile", "v8.parseOnBackground"]
                    and "args" in trace_event
                    and "data" in trace_event["args"]
                    and "url" in trace_event["args"]["data"]
                    and trace_event["args"]["data"]["url"].startswith("http")
                ):
                    e["js"] = trace_event["args"]["data"]["url"]
                if (
                    trace_event["name"] == "FunctionCall"
                    and "args" in trace_event
                    and "data" in trace_event["args"]
                ):
                    if "scriptName" in trace_event["args"]["data"] and trace_event[
                        "args"
                    ]["data"]["scriptName"].startswith("http"):
                        e["js"] = trace_event["args"]["data"]["scriptName"]
                    elif "url" in trace_event["args"]["data"] and trace_event["args"][
                        "data"
                    ]["url"].startswith("http"):
    ...
    def ProcessOldTimelineEvent(self, event, type):
        ...
        if start is not None and end is not None and end >= start and type is not None:
            ...
            if (
                ...
                "data" in event
                and "url" in event["data"]
                and event["data"]["url"].startswith("http")
            ):
                e["js"] = event["data"]["url"].split("#", 1)[0]
            elif (
                "data" in event
                and "scriptName" in event["data"]
                and event["data"]["scriptName"].startswith("http")
            ):
            ...
            if (
                "data" in event
                and "profile" in event["data"]
                and "rootNodes" in event["data"]["profile"]
            ):
    ...
    def ProcessTimelineEvents(self):
        if len(self.timeline_events) and self.end_time > self.start_time:
            ...
            if (
                self.interactive_end is not None
                and self.interactive_end - self.interactive_start > 500000
            ):
            ...
            for thread in self.cpu["slices"].keys():
                ...
                for name in self.cpu["slices"][thread].keys():
                    for slice in range(len(self.cpu["slices"][thread][name])):
                        self.cpu["slices"][thread][name][slice] = int(
    ...
    def ProcessTimelineEvent(self, timeline_event, parent, stack=None):
        ...
        if end > start:
            ...
            if (
                parent is None
                and elapsed > 50000
                and thread in self.cpu["main_threads"]
            ):
    ...
    def AdjustTimelineSlice(self, thread, slice_number, name, parent, elapsed):
        try:
            ...
            if name != parent:
                ...
                if (
                    parent is not None
                    and self.cpu["slices"][thread][parent][slice_number] >= fraction
                ):
                ...
                if self.cpu["slices"][thread]["total"][slice_number] > 1.0:
                    ...
                    for slice_name in self.cpu["slices"][thread].keys():
                        if slice_name != name:
                            self.cpu["slices"][thread][slice_name][slice_number] = min(
    ...
    def ProcessFeatureUsageEvent(self, trace_event):
        ...
        if (
            "name" in trace_event
            and "args" in trace_event
            and "feature" in trace_event["args"]
            and (
                trace_event["name"] == "FeatureFirstUsed"
                or trace_event["name"] == "CSSFirstUsed"
            )
        ):
            ...
            if trace_event["name"] == "FeatureFirstUsed":
                ...
                self.feature_usage["Features"][id]["firstUsed"].append(
                    trace_event["ts"]
                )
                ...
                if id not in self.feature_usage["CSSFeatures"]:
                    self.feature_usage["CSSFeatures"][id] = {
                        "name": name,
                        "firstUsed": [],
                    }
                self.feature_usage["CSSFeatures"][id]["firstUsed"].append(
                    trace_event["ts"]
                )
                ...
                if id not in self.feature_usage["AnimatedCSSFeatures"]:
                    self.feature_usage["AnimatedCSSFeatures"][id] = {
                        "name": name,
                        "firstUsed": [],
                    }
                self.feature_usage["AnimatedCSSFeatures"][id]["firstUsed"].append(
                    trace_event["ts"]
                )
            ...
            for category in self.feature_usage:
                ...
                for id in self.feature_usage[category]:
                    ...
                    for ts in self.feature_usage[category][id]["firstUsed"]:
                        timestamp = float(
                            "{0:0.3f}".format((ts - self.start_time) / 1000.0)
                        )
                        if timestamp > 0:
                    ...
                    if feature_time is not None:
                        out[category][id] = {
                            "name": self.feature_usage[category][id]["name"],
                            "firstUsed": feature_time,
                        }
        return out
    ...
    def ProcessNetlogEvent(self, trace_event):
        if (
            "args" in trace_event
            and "id" in trace_event
            and "name" in trace_event
            and "source_type" in trace_event["args"]
        ):
            try:
                if isinstance(trace_event["id"], (str, unicode)):
                    trace_event["id"] = int(trace_event["id"], 16)
                event_type = trace_event["args"]["source_type"]
                if event_type == "HOST_RESOLVER_IMPL_JOB" or trace_event[
                    "name"
                ].startswith("HOST_RESOLVER"):
                    self.ProcessNetlogDnsEvent(trace_event)
                elif (
                    event_type == "CONNECT_JOB"
                    or event_type == "SSL_CONNECT_JOB"
                    or event_type == "TRANSPORT_CONNECT_JOB"
                ):
    ...
    def post_process_netlog_events(self):
        ...
        if "url_request" in self.netlog:
            for request_id in self.netlog["url_request"]:
                ...
                if "url" not in request and "request_headers" in request:
                    ...
                    if "group" in request:
                        ...
                        "socket" in request
                        and "socket" in self.netlog
                        and request["socket"] in self.netlog["socket"]
                    ):
                    ...
                    for header in request["request_headers"]:
                        try:
                            ...
                            logging.exception(
                                "Error generating url from request headers"
                            )
                ...
                if (
                    ...
                    and not request["url"].startswith("http://192.168.10.")
                ):
                    # Match orphaned request streams with their h2 sessions
                    if (
                        "stream_id" in request
                        and "h2_session" not in request
                        and "url" in request
                    ):
                        ...
                        for h2_session_id in self.netlog["h2_session"]:
                            ...
                            if "host" in h2_session:
                                ...
                                if (
                                    "stream" in h2_session
                                    and request["stream_id"] in h2_session["stream"]
                                    and session_host == request_host
                                    and "request_headers" in request
                                    and "request_headers"
                                    in h2_session["stream"][request["stream_id"]]
                                ):
                                    ...
                                    if (
                                        ...
                                        and request_path == stream_path
                                    ):
                                        request["h2_session"] = h2_session_id
                                        break
                    # Copy any http/2 info over
                    if (
                        "h2_session" in self.netlog
                        and "h2_session" in request
                        and request["h2_session"] in self.netlog["h2_session"]
                    ):
            ...
            if len(requests):
                ...
                requests.sort(
                    key=lambda x: x["start"] if "start" in x else x["created"]
                )
                    ...
                    for request in requests:
                        if (
                            ...
                            if "claimed" not in socket:
                                ...
                                if (
                                    "h2_session" in request
                                    and request["h2_session"]
                                    in self.netlog["h2_session"]
                                ):
                                    h2_session = self.netlog["h2_session"][
                                        request["h2_session"]
                                    ]
                                    if "server_settings" in h2_session:
                                        request["http2_server_settings"] = h2_session[
                                            "server_settings"
                                        ]
                                ...
                                if "tls_cipher_suite" in socket:
                                    request["tls_cipher_suite"] = socket[
                                        "tls_cipher_suite"
                                    ]
                ...
                if "dns" in self.netlog:
                    ...
                    for request in requests:
                        if "connect_start" in request:
                            ...
                            if (
                                hostname in dns_lookups
                                and "claimed" not in dns_lookups[hostname]
                            ):
                                ...
                                if "times" in dns_lookups[hostname]:
                                    ...
                                    for dns in dns_lookups[hostname]["times"]:
                                        dns["end"] = min(
                                            dns["end"], request["connect_start"]
                                        )
                                        if dns["end"] >= dns["start"]:
                                            dns["elapsed"] = dns["end"] - dns["start"]
                                            if (
                                                elapsed is None
                                                or dns["elapsed"] > elapsed
                                            ):
                                                elapsed = dns["elapsed"]
                    ...
                    for time_name in times:
                        if time_name in request:
                            if (
                                ...
                                or request[time_name] < self.start_time
                            ):
                        ...
                        for key in ["chunks", "chunks_in", "chunks_out"]:
                            if key in request:
                                for chunk in request[key]:
                                    if "ts" in chunk:
                                        chunk["ts"] = (
                                            float(chunk["ts"] - self.start_time)
                                            / 1000.0
                                        )
                else:
                    requests = []
    ...
    def ProcessNetlogConnectJobEvent(self, trace_event):
        ...
        params = (
            trace_event["args"]["params"] if "params" in trace_event["args"] else {}
        )
    ...
    def ProcessNetlogStreamJobEvent(self, trace_event):
        ...
        if "source_dependency" in params and "id" in params["source_dependency"]:
            if name == "SOCKET_POOL_BOUND_TO_SOCKET":
                ...
                entry["socket"] = socket_id
                if (
                    "url_request" in entry
                    and entry["urlrequest"] in self.netlog["urlrequest"]
                ):
                    self.netlog["urlrequest"][entry["urlrequest"]]["socket"] = socket_id
                    if "group" in entry:
                        self.netlog["urlrequest"][entry["urlrequest"]]["group"] = entry[
                            "group"
                        ]
            ...
            if (
                ...
                or name == "HTTP2_SESSION_POOL_FOUND_EXISTING_SESSION"
                or name == "HTTP2_SESSION_POOL_FOUND_EXISTING_SESSION_FROM_IP_POOL"
            ):
                h2_session_id = params["source_dependency"]["id"]
                entry["h2_session"] = h2_session_id
                if (
                    h2_session_id in self.netlog["h2_session"]
                    and "socket" in self.netlog["h2_session"][h2_session_id]
                ):
                ...
                if (
                    ...
                    self.netlog["urlrequest"][entry["urlrequest"]]["h2_session"] = (
                        h2_session_id
                    )
    ...
    def ProcessNetlogHttp2SessionEvent(self, trace_event):
        ...
        if "stream_id" in params:
            ...
            if "url" in params:
                ...
                if "url_request" in stream:
                    ...
                    if (
                        ...
                        and request_id in self.netlog["url_request"]
                    ):
                        request = self.netlog["url_request"][request_id]
            ...
            if name == "HTTP2_SESSION_RECV_DATA" and "size" in params:
                ...
                stream["chunks"].append(
                    {"ts": trace_event["ts"], "bytes": params["size"]}
                )
            ...
            if (
                name == "HTTP2_STREAM_ADOPTED_PUSH_STREAM"
                and "url" in params
                and "url_request" in self.netlog
            ):
        ...
        if name == "HTTP2_SESSION_RECV_PUSH_PROMISE" and "promised_stream_id" in params:
            ...
            self.netlog["url_request"][request_id] = {
                "bytes_in": 0,
                "chunks": [],
            ...
            if "headers" in params:
                ...
                if scheme is not None and authority is not None and path is not None:
                    url = "{0}://{1}{2}".format(scheme, authority, path).split("#", 1)[
                        0
                    ]
                    request["url"] = url
                    stream["url"] = url
    ...
    def ProcessNetlogSocketEvent(self, trace_event):
        ...
        if (
            "connect_start" not in entry
            and name == "TCP_CONNECT_ATTEMPT"
            and trace_event["ph"] == "b"
        ):
            entry["connect_start"] = trace_event["ts"]
        ...
        if name == "SOCKET_BYTES_SENT" and "byte_count" in params:
            ...
            entry["chunks_out"].append(
                {"ts": trace_event["ts"], "bytes": params["byte_count"]}
            )
        if name == "SOCKET_BYTES_RECEIVED" and "byte_count" in params:
            entry["bytes_in"] += params["byte_count"]
            entry["chunks_in"].append(
                {"ts": trace_event["ts"], "bytes": params["byte_count"]}
            )
    ...
    def ProcessNetlogUrlRequestEvent(self, trace_event):
        ...
        if (
            "headers" in params
            and name == "HTTP_TRANSACTION_HTTP2_SEND_REQUEST_HEADERS"
        ):
            if isinstance(params["headers"], dict):
                entry["request_headers"] = []
                for key in params["headers"]:
                    entry["request_headers"].append(
                        "{0}: {1}".format(key, params["headers"][key])
                    )
            else:
                entry["request_headers"] = params["headers"]
    ...
    def ProcessV8Event(self, trace_event):
        try:
            if (
                ...
                and self.cpu["main_thread"] is not None
                and trace_event["ts"] >= self.start_time
                and "name" in trace_event
            ):
                thread = "{0}:{1}".format(trace_event["pid"], trace_event["tid"])
                if trace_event["ph"] == "B":
                    ...
                    if trace_event["ph"] == "E" and thread in self.v8stack:
                        ...
                        if (
                            start_event["name"] == trace_event["name"]
                            and "ts" in start_event
                            and start_event["ts"] <= trace_event["ts"]
                        ):
                    ...
                    self.v8stats["threads"][thread][name]["dur"] += (
                        float(duration) / 1000.0
                    )
                    self.v8stats["threads"][thread][name]["count"] += 1
                    if (
                        "args" in trace_event
                        and "runtime-call-stats" in trace_event["args"]
                    ):
                        for stat in trace_event["args"]["runtime-call-stats"]:
                            if (
                                len(trace_event["args"]["runtime-call-stats"][stat])
                                == 2
                            ):
                                if (
                                    "breakdown"
                                    not in self.v8stats["threads"][thread][name]
                                ):
                                    self.v8stats["threads"][thread][name][
                                        "breakdown"
                                    ] = {}
                                if (
                                    stat
                                ...
                                self.v8stats["threads"][thread][name]["breakdown"][
                                    ...
                                    float(
                                        trace_event["args"]["runtime-call-stats"][stat][
                                            1
                                        ]
                                    )
                                    / 1000.0
...
def main():
    ...
    parser.add_argument(
        "-l", "--timeline", help="Input timeline file (iOS or really old Chrome)."
    )
    ...
    parser.add_argument(
        "-x", "--longtasks", help="Output list of long main thread task times."
    )
