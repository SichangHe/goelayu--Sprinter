...
class WorkerThread(threading.Thread):
    ...
    def run(self):
        ...
        while (
            not self.__crawler.kill
            and self.is_alive()
            and TIMEOUT > (time.time() - self.start_time)
        ):
            try:
                    ...
                    if eachLink not in VISITED_URLS:
                        ...
                        try:
                            if not parsedURL.scheme and not parsedURL.netloc:
                                ...
                                newURL = urlunparse(
                                    parsedURL._replace(**{"scheme": urlObj.scheme})
                                )
                                eachLink = newURL
                            if (
                                eachLink not in VISITED_URLS
                            ):  # Check again for internal URL's
...
if __name__ == "__main__":
    try:
        ...
        print(
            "**************** Keyboard interrupt occured. Stopping all threads *******************"
        )
        exit(0)
    except Exception as e:
