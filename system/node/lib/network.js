...
function parseNetworkLogs(netLog) {
    ...
    for (var log of netLog) {
        ...
        switch (method) {
            case "Network.requestWillBeSent":
                if (requestId in requestIdToObject) {
                    ...
                    if (!("redirectResponse" in payLoad)) {
                        ...
                        prevReq.requestCS =
                            payLoad.redirectResponse.timing.sslEnd == -1
                                ? 0
                                : payLoad.redirectResponse.timing.sslEnd / 1000;
                        prevReq.requestFetch =
                            prevReq.requestStart +
                            payLoad.redirectResponse.timing.sendStart / 1000;
            ...
            case "Network.responseReceived":
                ...
                prevReq.requestCS = timing
                    ? timing.sslEnd == -1
                        ? 0
                        : timing.sslEnd / 1000
                    : 0;
                prevReq.requestFetch =
                    prevReq.requestStart + timing ? timing.sendStart / 1000 : 0;
    ...
    var sortedNetworkEntries = Object.entries(requestIdToObject)
        .map((e) => e[1])
        .sort((a, b) => {
...
function NetworkEvent(data) {
    ...
    data = isRequest
        ? data["Network.requestWillBeSent"]
        : data["Network.responseReceived"];
